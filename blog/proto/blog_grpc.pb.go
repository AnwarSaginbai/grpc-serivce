...
type BlogServiceClient interface {
	...
	ListsBlog(ctx context.Context, in *empty.Empty, opts ...grpc.CallOption) (BlogService_ListsBlogClient, error)
}
...
func (c *blogServiceClient) ListsBlog(ctx context.Context, in *empty.Empty, opts ...grpc.CallOption) (BlogService_ListsBlogClient, error) {
	stream, err := c.cc.NewStream(ctx, &BlogService_ServiceDesc.Streams[0], "/blog.BlogService/ListsBlog", opts...)
	if err != nil {
		return nil, err
	}
	x := &blogServiceListsBlogClient{stream}
...
type BlogService_ListsBlogClient interface {
	Recv() (*Blog, error)
	grpc.ClientStream
}

type blogServiceListsBlogClient struct {
	grpc.ClientStream
}

func (x *blogServiceListsBlogClient) Recv() (*Blog, error) {
	m := new(Blog)
...
type BlogServiceServer interface {
	...
	ListsBlog(*empty.Empty, BlogService_ListsBlogServer) error
...
func (UnimplementedBlogServiceServer) ListsBlog(*empty.Empty, BlogService_ListsBlogServer) error {
	return status.Errorf(codes.Unimplemented, "method ListsBlog not implemented")
}
...
func _BlogService_ListsBlog_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(empty.Empty)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(BlogServiceServer).ListsBlog(m, &blogServiceListsBlogServer{stream})
}

type BlogService_ListsBlogServer interface {
	Send(*Blog) error
	grpc.ServerStream
}

type blogServiceListsBlogServer struct {
	grpc.ServerStream
}

func (x *blogServiceListsBlogServer) Send(m *Blog) error {
...
var BlogService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "ListsBlog",
			Handler:       _BlogService_ListsBlog_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "blog.proto",
